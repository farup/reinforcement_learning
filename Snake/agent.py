...
class Agent():
    ...
    def get_state(self, game):
        ...
        # Boolean to check current direction
        dir_l = game.direction == Direction.LEFT
        dir_r = game.direction == Direction.RIGHT
        dir_u = game.direction == Direction.UP
        dir_d = game.direction == Direction.DOWN

        # example: if going right and point right of us gives an collision, then danger
        # states template:
        state = [
            # Danger straight
    ...
    def remember(self, state, action, reward, next_state, done):
        # poplef if memory is reached
        self.memory.append((state, action, reward, next_state, done))

    def train_long_memory(self):
        if len(self.memory) > BATCH_SIZE:
            mini_sample = random.sample(
                self.memory, BATCH_SIZE)  # return lis of tuples
        else:
    ...
    def get_action(self, state):
        ...
        # beggining do random, the exploit
